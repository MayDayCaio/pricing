...
Text Domain: woocommerce-time-based-pricing
...
function wctbp_admin_notices()
{
	...
	if($wctbp_notice && (!isset($_GET['page']) || $_GET['page'] != $wctbp_activator_slug))
	{
		...
		<div class="notice notice-success">
			<p><?php wctbp_html_escape_allowing_special_tag(sprintf(__( 'To complete the <span style="color:#96588a; font-weight:bold;">%s</span> plugin activation, you must verify your purchase license. Click <a href="%s">here</a> to verify it.', 'woocommerce-time-based-pricing' ), $wctbp_name, get_admin_url()."admin.php?page=".$wctbp_activator_slug)); ?></p>
		</div>
		<?php
	}
}
function wctbp_setup()
{
	...
	$wctbp_product_page, $wctbp_cart_item_fragment_addon, $wctbp_order_model;
	...
	if(!class_exists('WCTBP_Order'))
	{
		require_once('classes/com/WCTBP_Order.php');
		$wctbp_order_model = new WCTBP_Order();
	}
	...
	if(!class_exists('WCTBP_Checkout'))
	{
		require_once('classes/frontend/WCTBP_Checkout.php');
		new WCTBP_Checkout();
	}
