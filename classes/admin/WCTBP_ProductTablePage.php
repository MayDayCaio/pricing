...
class WCTBP_ProductTablePage
{
	...
	function manage_product_schedule_column( $column, $post_id ) 
	{
		...
		$general_title = '<strong>'.esc_html__('General rules: ', 'woocommerce-time-based-pricing').'</strong><br/>';;
		...
		if(($variations = $wctbp_product_model->get_variations($post_id) ) != null)
		{
			foreach((array)$variations as $variation)
			{
				...
				foreach((array)$rules2 as $rule)
				{
					...
					esc_html_e('(ID: ','woocommerce-time-based-pricing');
		...
		if($specific_product_rules && is_array($specific_product_rules))
		{
			...
			echo '<strong>'.esc_html__('Specific product rules: ', 'woocommerce-time-based-pricing').'</strong><br/>';
	...
	function add_post_schedule_info_column($columns)
	 {
	   ...
	   //add column
	   $columns['wctbp-pricing-rules'] =esc_html__('Pricing rule(s)', 'woocommerce-time-based-pricing'); 
