...
class WCTBP_Tax
{
	...
	static function get_product_price_including_tax($product, $quantity = '', $price = '')
	{
		...
		if(function_exists ('wc_get_price_including_tax'))
			return wc_get_price_including_tax($product, $args) != ""  ? wc_get_price_including_tax($product, $args): 1;
			
		return $product->get_price_including_tax($quantity, $price) != "" ? $product->get_price_including_tax($quantity, $price) : 1;
	}
	static function get_product_price_excluding_tax($product, $quantity = '', $price = '')
	{
		$args = array('qty' => $quantity, 'price' => $price);
		if(function_exists ('wc_get_price_excluding_tax'))
			return wc_get_price_excluding_tax($product, $args) != "" ? wc_get_price_excluding_tax($product, $args) : 1;
			
		return $product->get_price_excluding_tax($quantity, $price) != "" ? $product->get_price_excluding_tax($quantity, $price) : 1;
	}
}
?>