...
class WCTBP_Cart
{
	...
	public function cart_add_to_validation( $original_result, $product_id, $quantity , $variation_id = 0, $variations = null )
	{
		...
		if($wctbp_product_model->get_new_price_or_discount_rule(0, $product) === 'hide_price')
		{
			...
			wc_add_notice( esc_html__('You are not authorized to buy this product.','woocommerce-time-based-pricing') ,'error');
		}
	...
	public function cart_update_validation($original_result, $cart_item_key, $values, $quantity )
	{
		...
		{
			$product = wc_get_product($items[$cart_item_key]['product_id']);
			if(!$product)
				return $original_result;
		
			if($wctbp_product_model->get_new_price_or_discount_rule(0, $product) === 'hide_price')
			{
				wc_add_notice( sprintf(esc_html__('You are not authorized to buy %s. It has been removed from cart.','woocommerce-time-based-pricing'), "<strong>".$product->get_title( )."</strong>") ,'error');
				$original_result = false;
	...
	public function cart_validation_on_checkout()
	{
		...
		{
			$product = wc_get_product($items[$cart_key]['product_id']);
			if(!$product)
				return $original_result;
			if($wctbp_product_model->get_new_price_or_discount_rule(0, $product) === 'hide_price')
			{
				wc_add_notice( sprintf(esc_html__('You are not authorized to buy %s. It will be removed from the cart.','woocommerce-time-based-pricing'), "<strong>".$product->get_title( )."</strong>") ,'error');
	   ...
	   foreach((array)$discounts_array as $discount_rule)
	   {
		  ...
		  $apply_tax = $wctbp_option_model->get_option('wctpb_apply_tax_to_cart_discount','no') == 'yes' ? true : false;
		   if($discount_rule['type'] == 'cart_fixed' ) //subtract
		   {
				...
				$additional_text = wctbp_get_value_if_set($discount_rule, 'cart_label', esc_html__('Discount','woocommerce-time-based-pricing'));
		   }
		    if($discount_rule['type'] == 'cart_fixed_add_fee' )
		   {
				$discount = $discount_rule['value']*$discount_rule['times_to_apply_discount'];
				$additional_text = wctbp_get_value_if_set($discount_rule, 'cart_label', esc_html__('Fee','woocommerce-time-based-pricing'));
		   }
		   elseif($discount_rule['type'] == 'cart_percentage')
		   {
			   ...
			   $additional_text = wctbp_get_value_if_set($discount_rule, 'cart_label', esc_html__('Discount','woocommerce-time-based-pricing'))." ({$discount_rule['value']}%)";
		   }
		   elseif($discount_rule['type'] == 'cart_percentage_add_fee')
		   {
			   $discount = $woocommerce->cart->subtotal_ex_tax*($discount_rule['value']/100); 
			   $discount = $discount > 0 ? round($discount, wc_get_price_decimals()) : $discount; //ceil
			   $additional_text = wctbp_get_value_if_set($discount_rule, 'cart_label', esc_html__('Fee','woocommerce-time-based-pricing'))." ({$discount_rule['value']}%)";
		   }
			   ...
			   foreach((array)$discount_rule['cart_item'] as $cart_item)
				{
					...
					if( $cart_item["data"]->get_type() == "variation" && $cart_item["data"]->get_id() != "" && $cart_item["data"]->get_id() != 0)
					{
						$name = $wctbp_product_model->get_variation_complete_name( $cart_item["data"]->get_id() , $discount_rule['cart_item']);
					}
					...
					$additional_text = wctbp_get_value_if_set($discount_rule, 'cart_label', esc_html__('Discount','woocommerce-time-based-pricing'))." ({$fee_quantity_label}X - ".$name.")";
					...
					$woocommerce->cart->add_fee($additional_text, $discount * -1 ,  $apply_tax ); 
		  ...
		  $this->total_saved +=  $discount_to_subtract < 0 ? $this->get_fee_value($discount, true) : 0;
		   $woocommerce->cart->add_fee($additional_text, $discount_to_subtract,  $apply_tax ); 
	   }
