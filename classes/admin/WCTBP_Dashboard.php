...
class WCTBP_Dashboard
{
	...
	public function add_server_time_widget()
	{
		if(current_user_can('manage_woocommerce') || current_user_can('edit_posts'))
			wp_add_dashboard_widget( 'wctbp-server-time', esc_html__('WooCommerce Pricing! - Server time', 'woocommerce-time-based-pricing'), array( &$this, 'render_server_time_widget' ));
	...
	function render_server_time_widget()
	{
		...
		?>
		<p class="form-field">
			<label  style="display: inline;"><?php echo esc_html__( 'Current server time with offset (date format: dd/mm/yyyy):', 'woocommerce-time-based-pricing' ); ?></label>
			<span class="wrap">
